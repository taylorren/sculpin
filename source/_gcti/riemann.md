...
对莱布尼茨而言，定积分\\(\int y(x)dx\\)是无限多个微小的部分之和：\\(\sum y_i(x)\Delta _i (x)\\)

![]({{site.url}}/assets/img/gcti/1503.png)
...
![]({{site.url}}/assets/img/gcti/1504.png)

反之，黎曼要求每个区间内的任意值都可以用来进行黎曼求和，而且当区间范值趋向0的时候，必定收敛到一个极限。

如下是黎曼求和示意图（取任意点的值）。

![]({{site.url}}/assets/img/gcti/1505.png)

函数f(x)在区间[a, b]上存在柯西积分，取决于区间[a, b]区间内f(x)的连续性。黎曼积分存在的要求依赖一个与之关联但是差别巨大的概念：变分。和柯西一样，黎曼一开始先将区间[a, b]分成由一系列点\\(x_1, x_2, x_3, ..., x_{n-1}\\), xn构成的区间，且\\(a=x_0<x_1<x_2<x_3<...<x_{n-1}<x_n=b\\)。然后他考虑函数f(x)在每个小区间![](img/1507.png)上函数值的变化。也就是在各个子区间![](img/1507.png)上函数值变化的最大值：\\(D_k=max(f(s)-f(t))\\)
...
![]({{site.url}}/assets/img/gcti/1513.png)
